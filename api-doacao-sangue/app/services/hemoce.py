...
URL = "https://www.hemoce.ce.gov.br/"

TIPOS = ["O+", "O-", "A+", "A-", "B+", "B-", "AB+", "AB-"]


def coletar_estoque_hemoce():
    try:
        ...
        print("STATUS HEMOCE:", response.status_code)
        response.raise_for_status()

        soup = BeautifulSoup(response.text, "html.parser")
        texto = soup.get_text(" ", strip=True).lower()

        print("TEXTO HEMOCE:", texto[:1500])
        ...
        if "todos os tipos" in texto and (
            "baixo" in texto or "baixa" in texto or "crítico" in texto or "critico" in texto
        ):
            for tipo in TIPOS:
                estoque[tipo] = "Alerta"

        print("ESTOQUE HEMOCE:", estoque)
