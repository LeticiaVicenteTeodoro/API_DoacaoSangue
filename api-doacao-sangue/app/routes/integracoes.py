...
from app.services.hemoba import coletar_estoque_hemoba
from app.services.hemosul import coletar_estoque_hemosul
...
@router.post("/hemoba/estoques")
def atualizar_estoque_hemoba(
    db: Session = Depends(get_db)
):
    dados = coletar_estoque_hemoba()

    if not dados:
        raise HTTPException(
            status_code=500,
            detail="Não foi possível coletar dados do HEMOBA"
        )

    hemocentro = db.query(Hemocentro).filter(
        Hemocentro.nome == "HEMOBA",
        Hemocentro.estado == "BA"
    ).first()
    ...
    if not hemocentro:
        hemocentro = Hemocentro(
            ...
            estado="BA",
            cidade="Salvador",
            site="https://www.hemoba.ba.gov.br/"
        )
...
@router.post("/hemosul/estoques")
def atualizar_estoque_hemosul(
    db: Session = Depends(get_db)
):
    dados = coletar_estoque_hemosul()

    if not dados:
        raise HTTPException(
            status_code=500,
            detail="Não foi possível coletar dados do HEMOSUL"
        )

    hemocentro = db.query(Hemocentro).filter(
        Hemocentro.nome == "HEMOSUL",
        Hemocentro.estado == "MS"
    ).first()
    ...
    if not hemocentro:
        hemocentro = Hemocentro(
            ...
            estado="MS",
            cidade="Campo Grande",
            site="https://www.hemosul.ms.gov.br/"
        )
